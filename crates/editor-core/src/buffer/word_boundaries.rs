...
mod tests {
    ...
    fn test_simple_words() {
        let finder = WordBoundaryFinder::new("hello world");
        let boundaries = finder.boundaries();
    ...
    fn test_next_word_boundary() {
        let finder = WordBoundaryFinder::new("hello world test");

        let next = finder.next_word_boundary(0);
        assert!(next.is_some());

        let next = finder.next_word_boundary(5);
        assert!(next.is_some());
    }
    ...
    fn test_prev_word_boundary() {
        ...
        let prev = finder.prev_word_boundary(10);
        assert!(prev.is_some());
    }
    ...
    fn test_word_at() {
        ...
        let word = finder.word_at(2);
        assert_eq!(word, Some("hello".to_string()));

        let word = finder.word_at(7);
        assert_eq!(word, Some("world".to_string()));
    }

    #[test]
    fn test_unicode_words() {
        let finder = WordBoundaryFinder::new("hello שלום world");
        let word = finder.word_at(6);
        assert!(word.is_some());
    }
    ...
    fn test_punctuation() {
        ...
        // Should have boundaries around punctuation
        let punct_boundaries: Vec<_> = boundaries
            .iter()
            .filter(|b| b.boundary_type == BoundaryType::Punctuation)
            .collect();
        assert!(punct_boundaries.len() > 0);
    }
}
