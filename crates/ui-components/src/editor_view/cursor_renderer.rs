...
mod tests {
    ...
    fn test_cursor_renderer_creation() {
        ...
        assert_eq!(renderer.style(), CursorStyle::Line);
        assert!(renderer.is_primary_visible());
    }

    #[test]
    fn test_cursor_style_change() {
        let mut renderer = CursorRenderer::new();
        renderer.set_style(CursorStyle::Block);
        assert_eq!(renderer.style(), CursorStyle::Block);
    }

    #[test]
    fn test_blink_disable() {
        let mut renderer = CursorRenderer::new();
        renderer.set_blink_enabled(false);
        assert!(renderer.is_primary_visible());

        // Even after update, should stay visible
        renderer.update_blink();
        assert!(renderer.is_primary_visible());
    }

    #[test]
    fn test_cursor_moved_resets_blink() {
        let mut renderer = CursorRenderer::new();
    ...
    #[test]
    fn test_secondary_cursors() {
        let mut cursors = SecondaryCursors::new();
        assert!(cursors.is_empty());

        cursors.add(Position::new(0, 0));
        cursors.add(Position::new(1, 5));
    ...
    #[test]
    fn test_primary_cursor() {
        let mut cursor = PrimaryCursor::new(Position::new(0, 0));
