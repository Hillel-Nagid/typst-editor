...
/*
TODO: render RTL properly 
	Current Issues:
	- RTL only lines are not being rendered at all
	- RTL highlighting is completely broken
	- RTL cursor is not moved
	- RTL selection is not selecting correctly
	*/
pub fn editor_view(
    theme: &Theme,
    ...
    let (lines, visual_lines, directions, line_selections, line_cursor_cols, cursor_line) = if
        let Some(workspace) = state.read().get_active_workspace()
    {
        ...
        if let Some(editor) = workspace.get_active_editor() {
            ...
            let line_ranges = compute_line_content_ranges(
                &line_start_offsets,
                content.chars().count()
            );
            let line_selections = compute_line_selections(
                &lines,
                &line_start_offsets,
                &line_ranges,
                &cursors
            );
            let line_cursor_cols = compute_line_cursor_visual_cols(
                &lines,
                &line_start_offsets,
                &cursors,
                &visual_lines
            );
            let (line_idx, _) = offset_to_line_col(
                &line_start_offsets,
                cursors[editor.cursors().primary_index()].position()
            );
            (lines, visual_lines, directions, line_selections, line_cursor_cols, line_idx)
        } else {
            (
...
fn display_col_from_global_offset(line: &str, line_start: usize, offset: usize) -> usize {
    offset.saturating_sub(line_start).min(line.chars().count())
}
    ...
    for cursor in cursors {
        ...
        for line_idx in 0..lines.len() {
            ...
            if overlap_start < overlap_end {
                let start_col = display_col_from_global_offset(
                    &lines[line_idx],
                    line_starts[line_idx],
                    overlap_start
                );
                let end_col = display_col_from_global_offset(
                    &lines[line_idx],
                    line_starts[line_idx],
                    overlap_end
                );
...
fn compute_line_cursor_visual_cols(
    ...
    for cursor in cursors {
        ...
        let logical_col = display_col_from_global_offset(
            &lines[line_idx],
            line_starts[line_idx],
            position
        );
...
fn render_visual_line_with_cursors_and_selection(
    ...
    for col in &cursor_cols {
        boundaries.push((*col).min(display_len));
    }
    boundaries.sort_unstable();
    boundaries.dedup();

    for boundary_index in 0..boundaries.len() {
        let col = boundaries[boundary_index];
        if cursor_cols.iter().any(|cursor_col| *cursor_col == col) {
        ...
        if boundary_index + 1 < boundaries.len() {
            let next_col = boundaries[boundary_index + 1];
            if next_col <= col {
                continue;
            }
            let segment: String = visual_chars[col..next_col].iter().collect();
            let selected = selections.iter().any(|range| col >= range.start && col < range.end);
            row_widget = row_widget.push(
                container(text(segment).size(14).shaping(iced::widget::text::Shaping::Advanced))
